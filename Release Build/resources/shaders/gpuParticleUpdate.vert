#version 410
	...
	void main() 
	{
		...
		velocity.x = (rand(seed++, 2) - 1) + (sin(time / 2));
		velocity.y = (rand(seed++, 2) - 1) + abs(cos(time / 2));	//(rand(seed++, 2) - 1) + 0;
		velocity.z = (rand(seed++, 2) - 1) + 0;//(rand(seed++, 2) - 1) + (cos(time / 2));//(rand(seed++, 2) - 1) + 0;//(rand(seed++, 2) - 1) + cos(time);		
