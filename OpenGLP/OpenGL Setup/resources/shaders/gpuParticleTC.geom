#version 410
	...
	uniform vec4 colourMid;
	
	void main() 
	{
		// interpolate colour
		// less than halfway through life
		if ( (lifetime[0] / (lifespan[0])) < 0.5f)
			Colour = mix(colourStart, colourMid, ((lifetime[0] / (lifespan[0])) * 2));
			
		else
			Colour = mix(colourMid, colourEnd, ((lifetime[0] / (lifespan[0] - (lifespan[0] / 15))) * 2) - 1.0f);
