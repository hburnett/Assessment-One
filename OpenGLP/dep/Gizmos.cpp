								...
								float a_latMin /*= -90*/, float a_latMax /*= 90*/)
{
	...
	for (int face = 0; face < (a_rows)*(a_columns); ++face )
	{
		...
		addLine(a_center + v4Array[face], a_center + v4Array[face+a_columns], glm::vec4(1.f,1.f,1.f,.0f), glm::vec4(1.f,1.f,1.f,.0f));
		
		if( face % a_columns == 0 && longitudinalRange < (glm::pi<float>() * 2))
		{
				continue;
		}
		addLine(a_center + v4Array[iNextFace+a_columns], a_center + v4Array[face+a_columns], glm::vec4(1.f,1.f,1.f,.0f), glm::vec4(1.f,1.f,1.f,.0f));
